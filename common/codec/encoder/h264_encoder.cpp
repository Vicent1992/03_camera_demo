...
H264Encoder::H264Encoder() : extra_data_(nullptr), extra_data_size_(0) {
...
int H264Encoder::InitConfig(MediaConfig& config) {
  ...
  if (pic_type == MPP_FMT_YUV422_YUYV || pic_type == MPP_FMT_YUV422_UYVY)
    prep_cfg->hor_stride = vconfig.width * 2;
  else
...
int H264Encoder::CheckConfigChange() {
...
int H264Encoder::EncodeOneFrame(IonBuffer* src, IonBuffer* dst, IonBuffer* mv) {
  int ret = 0;
  VideoConfig& vconfig = GetVideoConfig();
  ...
  if (vconfig.fmt == PIX_FMT_YVYU422 || vconfig.fmt == PIX_FMT_UYVY422)
    mpp_frame_set_hor_stride(rkenc_ctx->frame, UPALIGNTO16(vconfig.width * 2));
  else
...
int H264Encoder::EncodeControl(int cmd, void* param) {
  int ret = 0;
  RkMppEncContext* rkenc_ctx = &mpp_enc_ctx_;
