...
int vpu_nv12_encode_jpeg_init_ext(struct vpu_encode* encode,
  ...
  if (ion_manage_alloc(&encode->jpeg_enc_out, width, height, 3, 2)) {
...
int vpu_nv12_encode_jpeg_doing(struct vpu_encode* encode,
                               ...
                               size_t src_size) {
  MPP_RET ret = MPP_OK;
  unsigned int width = encode->width;
  unsigned int height = encode->height;
  MppCtx mpp_ctx = encode->mpp_ctx;
  MppApi* mpi = encode->mpi;
  MppTask task = NULL;
  MppFrame frame = NULL;
  ...
  if (src_fd > 0) {
    ...
    memcpy((unsigned char*)mpp_buffer_get_ptr(pic_buf), srcbuf, src_size);
  }
  ...
  if ((encode->packet != NULL)) {
    ...
    encode->enc_out_data = (unsigned char *)encode->jpeg_enc_out.buffer;
...
void vpu_nv12_encode_jpeg_done(struct vpu_encode* encode) {
  if (encode) {
    ...
    ion_manage_free(&encode->jpeg_enc_out);
  }
}
...
int vpu_decode_jpeg_doing(struct vpu_decode* decode,
  ...
  int width = decode->in_width;
  int height = decode->in_height;
  ...
  memcpy((unsigned char*)mpp_buffer_get_ptr(str_buf), in_data, decode->pkt_size);
